...
- [x] **Step 2: Implement fixed-range config read/write**
...
- [x] **Step 3: Implement manager/employee authorization with negative proof**
...
- [x] **Step 4: Commit config and access policy**
...
- [x] **Step 2: Implement deterministic Sheets API template requests**
...
- [x] **Step 3: Write failing create-service orchestration tests**
...
- [x] **Step 4: Implement create service and authenticated Route Handler**
...
- [x] **Step 5: Commit monthly creation**
...
- [x] **Step 3: Write failing import orchestration and recovery tests**
...
- [x] **Step 4: Implement multipart routes and import service**
...
- [x] **Step 5: Commit import support**
...
- [x] **Step 1: Write failing manager and employee discovery tests**
...
- [x] **Step 2: Implement server discovery and browser-only folder preference**
...
- [x] **Step 3: Implement dashboard API and English UI states**
...
- [x] **Step 4: Implement explicit legacy-file setup with same-file proof**
...
- [x] **Step 5: Commit discovery and dashboard**
...
- [x] **Step 1: Write failing create-wizard interaction tests**
...
- [x] **Step 2: Implement accessible reusable member rows and create wizard**
...
- [x] **Step 3: Write failing import-wizard interaction tests**
...
- [x] **Step 4: Implement import wizard and shared API error presentation**
...
- [x] **Step 5: Commit manager wizards**
...
- [x] **Step 1: Write failing owner-only and idempotency tests**
...
- [x] **Step 2: Implement add-member service and API**
...
- [x] **Step 3: Implement English member page and retry UI**
...
- [x] **Step 4: Commit member management**
...
- [x] **Step 1: Write failing read and authorization tests**
...
- [x] **Step 2: Write failing exact-save and conflict-disclosure tests**
...
- [x] **Step 3: Implement date-to-row resolution and allowed-range enforcement**
...
- [x] **Step 4: Implement safe Route Handler responses**
...
- [x] **Step 5: Commit attendance API**
...
- [x] **Step 1: Write failing combined-editor interaction tests**
...
- [x] **Step 2: Write failing lunch, notes, calculation, and retry tests**
...
- [x] **Step 3: Implement one-reducer editor and focused components**
...
- [x] **Step 4: Run focused UI and full component proof**
...
- [x] **Step 1: Write and prove the production test-bypass guard before adding the adapter**
...
- [x] **Step 2: Add a deterministic non-production session/Google adapter**
...
- [x] **Step 3: Implement Playwright workflows against the real UI/API**
...
- [x] **Step 4: Add optional proof against the supplied reference workbook**
...
- [x] **Step 5: Write product, Docker, and Google Cloud runbooks**
...
- [x] **Step 6: Run the complete local and Docker verification matrix**
...
- [x] **Step 7: Record live limits, update plan result, and commit**
...
Task 5 Steps 2-3 RED/GREEN proof: `docker compose run --rm test npm test -- src/lib/config src/lib/access` first failed to resolve `./repository` and `./policy`; GREEN passed 69/69 across schema, repository, and policy. `read` detects a missing `__APP_CONFIG` from the spreadsheet snapshot and performs zero value reads before failing. `initialize` order is snapshot, add hidden sheet, write the three ranges, add the owner-only protected range, then set appProperties; it refuses to overwrite an existing config sheet unless the caller passes `replaceExisting: true`, and `deleteSheet` is emitted on no other path. Sheet and protection identifiers are read from `BatchUpdateResult.replies` by key rather than by index. `updateMemberProgress` and `updateSetupState` each write exactly one narrow range. Authorization proves a non-owner cannot obtain the manager role, a null or stale mapping raises `NeedsSetupError`/`NeedsRepairError` instead of falling back to a title match, and employee A requesting employee B's sheet raises `ForbiddenError` whose serialized form was asserted to contain no other member's title, email, sheet ID, or protection ID.
Task 6 Steps 1-2 RED/GREEN proof: `docker compose run --rm test npm test -- src/lib/workbook/template.test.ts src/lib/files` first failed to resolve `./template` and `./schemas`; GREEN passed 34/34. The July 2026 plan produces 31 rows from row 4, business-day numbering blank on weekends, `formulaValue` `=F{row}-G{row}-E{row}` rather than computed hours, `hourMerges` from `J2:K2` to `AR2:AS2`, 36 alternating minute headers, a `{ rows: 3, columns: 2 }` frozen pane, status list validation restricted to business-day column D cells, and `NUMBER`/`0.##` formatting on E:H so decimal hours are never coerced into time fractions. Every request addresses the numeric sheet ID; no sheet title appears in a payload. Sheet titles are rejected when empty, reserved, over 100 characters, duplicated after normalization, or containing `: \ / ? * [ ]`.
Task 8 partial proof: `docker compose run --rm test npm test -- src/lib/dashboard` first failed to resolve `./folder-preference`; GREEN passed 16/16. The browser preference stores exactly `id` and `name` under an email-scoped key, removes malformed JSON and structurally invalid values, replaces rather than accumulates, and degrades to no preference when `localStorage` itself throws, which also makes it safe during server rendering. Discovery, the dashboard API, and the dashboard UI remain unimplemented.
Wave 2 validation: `docker compose run --rm test npm run verify` passed end to end after Tasks 5 Steps 2-3, 6 Steps 1-2, and the folder preference landed together (lint, typecheck, Vitest 210/210 across 20 files, production build). `googleapis` is still imported by `src/lib/google/client.ts` only. The three streams ran concurrently under disjoint file ownership and `git status` confirmed no cross-stream modification.
Task 6 Steps 3-5 proof: `docker compose run --rm test npm test -- src/lib/files/setup-service.test.ts` passed 12/12 and `docker compose run --rm test npm test -- src/app/api/files/create` passed 7/7. The service proves the ordered create sequence, rejects duplicate member emails before any Google mutation with an empty event log, and retains file, folder, and member progress on a failed invitation so retry neither recreates the file, tab, or protection nor re-invites a completed member. The route answers 201 when setup completes and 207 with retained IDs when it does not, strips a client-supplied `ownerEmail` at the schema boundary so the protected config records the session identity, and answers 401 and 400 without reaching Google.
Task 8 Steps 1-3 proof: `docker compose run --rm test npm test -- src/lib/discovery` passed 21/21. Manager discovery returns only direct children of the validated folder, excludes nested and wrongly named files, and marks a legacy file `needs-setup`; employee discovery requires `sharedWithMe`, a case-insensitive owner domain, a case-sensitive filename marker, and exactly one config mapping, excluding zero or two mappings, a missing sheet, and an out-of-domain owner. Config is read per candidate sequentially so one unreadable file becomes a card-level error instead of failing the dashboard. `GET /api/dashboard` re-derives the actor from the session and ignores any client-supplied email.
Wave 3 interruption and recovery: four concurrent streams were cut off mid-task by an API session limit. Task 6 and Task 8 had complete service layers and were finished by hand; the create route and the dashboard page were the only missing files. The Task 10 and Task 11 streams had produced tests but no implementation, so their RED specifications were preserved outside the repository and their partial files removed, and the uncommitted edit to `src/lib/google/sheets-gateway.test.ts` was reverted to its committed state. No partially implemented module was committed.
Two defects were found by the gate rather than by focused tests, which is why the whole-project gate is not optional. First, `dashboard-client.tsx` called `setState` inside its mount effect through a helper, which `react-hooks/set-state-in-effect` rejects; the initial load now sets state from the promise continuation and discards a superseded run, so a slow response for a previous email cannot overwrite a newer one, while the user-initiated reload keeps its synchronous `loading` reset. Second, Vitest runs without `globals`, so Testing Library never registered automatic cleanup and every render accumulated in the document, which failed 11 dashboard tests with duplicated elements; `vitest.setup.ts` now runs `cleanup` after each test, which every future component test depends on.
Wave 3 validation: `docker compose run --rm test npm run verify` exited 0 with lint, typecheck, Vitest 280/280 across 25 files, and the production build. The exit status was captured directly rather than read from the end of a shell pipeline.
Task 7 Steps 3-5 proof: `docker compose run --rm test npm test -- src/lib/files/import-service.test.ts src/app/api/files/import` passed 32/32. Inspection performs no Drive or Sheets call; save validates every sheet mapping, revalidates the folder, converts the original bytes unchanged with exactly one parent, replaces the untrusted uploaded `__APP_CONFIG` through `replaceExisting: true`, protects the imported tabs, invites sequentially, and answers the plan's literal 207 shape with `retryable: true`. Retry reuses the converted file and never uploads again. Because the initial RED was module resolution only, two deliberate mutations proved the key assertions bite: moving folder validation ahead of input validation failed the three no-Google-call tests, and replaying the employee template onto imported tabs failed the grid-resize guard. Both were reverted.
Task 10 proof: `docker compose run --rm test npm test -- src/lib/files/member-service.test.ts 'src/app/api/files/[fileId]/members' 'src/app/(authenticated)/files'` passed 38/38. An employee is refused before any mutation with an empty event log, an existing member email or normalized sheet title returns 409, invitations are serialized, and retry short-circuits when a permission already exists rather than sharing twice. No delete, removal, or revocation path exists in the service, the route, or the UI.
Task 11 proof: `docker compose run --rm test npm test -- src/lib/attendance src/lib/google 'src/app/api/files/[fileId]/attendance'` passed 81/81. Employee A addressing employee B's sheet returns 403 with zero attendance value reads and no disclosure of the other member. Columns A, B, C, and H are unaddressable by construction because no field key maps to them rather than by a deny list; the row is resolved from column A server-side; dirty cells are re-read immediately before the write; a same-cell divergence is disclosed as a conflict while a different-cell change causes neither a conflict nor a row-wide write.
Free-text corruption fix: `sheets-gateway` previously sent every write as `USER_ENTERED`, which is required for the `=F-G-E` contract but silently converted a note beginning with `=` into a formula and a note like `2026-07` into a date. `ValuePatch` now carries an optional input option defaulting to `USER_ENTERED`, the gateway groups patches by option, and the attendance save sends column I and the J:AS work slots as `RAW`. The pre-existing gateway test asserting a single `USER_ENTERED` batch still passes unmodified, and a note of `=1+1` is now stored literally.
Member status vocabulary correction: three services wrote member `setupStatus` into the same config column, and the create service had invented `complete` while the plan states `pending`, `ready`, and `invite-failed` in both Task 7 Step 3 and Task 10 Step 2. The create service, its tests, and the create route test were realigned to `ready`, and the member service's compatibility mapping for the invented value was removed, so one vocabulary now reaches the protected config.
Wave 4 validation: `docker compose run --rm test npm run verify` exited 0 with lint, typecheck, Vitest 388/388 across 33 files, and the production build.
Task 8 Step 4 proof: `docker compose run --rm test npm test -- src/lib/files/setup-service.test.ts 'src/app/api/files/[fileId]/setup' 'src/app/(authenticated)/files/[fileId]/setup'` passed 48/48, and a regression sweep over the files, config, access, discovery, and file-route suites passed 217/217. `configureExisting` reuses the existing tabs and never calls the employee template builder, asserted by an empty grid-resize log, so an imported or legacy tab can never be truncated. A picked file ID that differs from the route ID is refused with 403 before the service is constructed, duplicate mappings return 400, and a retry that re-points an already configured member at a different tab returns 409 rather than orphaning its recorded protection. The create path is untouched: a separate `LegacySetupError` union was added instead of widening `SetupErrorCode`, which the committed create route consumes as an exhaustive record.
Task 12 proof: `docker compose run --rm test npm test -- src/components 'src/app/(authenticated)/files/[fileId]'` passed 77/77, of which the editor and its three components contribute 53. Both editing modes render one reducer draft, so a timeline edit and a work-block edit cannot disagree. The client does not restate what changed: it runs the domain's own `diffDay` at a sentinel row and maps the resulting A1 columns back to wire field keys, so its dirty set cannot drift from the server's. Lunch reservation, block application, work-hour calculation, and validation all reuse the committed pure domain rather than reimplementing the rules in components.
Task 9 proof: `docker compose run --rm test npm test -- 'src/app/(authenticated)/files/new' 'src/app/(authenticated)/files/import' src/components/member-rows.test.tsx` passed 61/61. The create wizard exposes a resume link for a 207 because the create endpoint accepts no resume hint and re-posting would create a second file; the import wizard retries in place by resending the same upload with `resumeFileId`, proved by a second save call carrying the resume ID while the inspect call count stays at one. Multipart field names are local constants rather than imports from the import schema module, which transitively pulls ExcelJS and would otherwise reach the browser bundle, while sheet-title rules are imported from the pure workbook module so client and server refuse identical titles.
Dashboard entry points: the wizards shipped unreachable — no dashboard control linked to `/files/new` or `/files/import`. Both were added to the managed section header. They use `next/link`, because the repository lint rule rejects a plain anchor for a statically known page route; the existing card links escape that rule only because their hrefs are dynamic.
Wave 5 validation: `docker compose run --rm test npm run verify` exited 0 with lint, typecheck, Vitest 538/538 across 42 files, and the production build.
Task 13 Steps 1-2 proof: `docker compose run --rm test npm test -- src/lib/testing src/app/api/e2e` first failed to resolve `./runtime-guard`; GREEN passed 25/25. `resolveTestMode` takes its environment as an argument, throws `E2E_TEST_MODE is forbidden in production` when the flag is present under `NODE_ENV=production`, returns false in production without it, and treats an unrecognized `NODE_ENV` as off, so a mis-set variable fails closed. The shared secret is compared in constant time and an unset or empty configured secret can never match, so forgetting to configure it locks the control route instead of opening it. `POST /api/e2e/reset` answers 404 with an empty body for every invalid case rather than confirming the route exists, and the production refusal is proven deterministically because a browser cannot host a production build.
Task 13 Step 3 proof: `docker compose run --rm test npm run test:e2e` passed 26/26 against the real UI and API, cold and warm. Only the external Google Picker callback is mocked; the application's own endpoints are exercised. Coverage includes folder selection and change, `Folder unavailable` with no all-Drive fallback, create and import, explicit Picker confirmation before legacy setup, employee-scoped discovery, the block/timeline/lunch/notes/status/save/retry path, and an employee receiving 403 for another member's sheet URL and API.
Task 13 Step 4 proof: the reference workbook is not inside this worktree, so the plan's `/app/202607勤怠管理表.xlsx` path does not exist in the container and the 391 KB binary was deliberately not copied into the repository. It was mounted read-only instead. `docker compose run --rm -v "<repo parent>/202607勤怠管理表.xlsx:/ref.xlsx:ro" --env REFERENCE_XLSX_PATH=/ref.xlsx test npm test -- tests/reference-workbook.test.ts` passed 13/13; without the variable the same file reports 13 skipped rather than a spurious pass. The four observed employee sheets are `THAI GIA HAN`, `NGUYEN PHAN LINH`, `NGUYEN THI NHU HIEU`, and `NGUYEN HO TRONG THAO`, each resolving to `2026-07` with 31 date rows, the exact `D3:I3` headers, and the `J:AS` grid. The real workbook disproved a first-draft assertion: column H is blank on non-business days rather than carrying a formula on all 31 rows, which the generated fixture would never have surfaced. The test now asserts blank-or-`F{r}-G{r}-E{r}` with blanks required to coincide with blank clock values, matching the inspector's reconcilable rule. `vitest.config.ts` previously collected only `src/**`, so this suite was unreachable from `npm test`; `tests/**/*.test.ts` was added to the include list while `tests/e2e/**` stays excluded.
Task 13 Step 6 verification matrix, each command run separately with its own exit status captured: `docker compose build test` 0; `npm run lint` 0; `npm run typecheck` 0; `npm test` 0 with 563 passed and 13 skipped across 45 files; `npm run test:e2e` 0 with 26 passed; `docker compose build app` 0; `docker compose up --detach app` 0 with the container reporting `healthy`; `curl --fail http://localhost:3000/api/health` returned `{"status":"ok"}` with exit 0; `docker compose down` 0, after which `docker compose ps -a` listed nothing for this project. `docker compose config` shows credentials resolved from the env file into the service environment at run time, and `docker history` on `google-sheets-attendance-app:latest` contains no occurrence of `AUTH_SECRET`, so no secret is baked into an image layer.
Size-ceiling refactor: `fake-google-store.ts` (907), `setup-service.ts` (865), and `attendance-editor.tsx` (826) exceeded the repository's 800-line maximum. They were split along cohesive seams into 360, 61, and 348 lines with eleven new modules, none over 360. No test file was edited, which is the proof the refactor changed no behavior: the gate reported the same 563 passed and 13 skipped before and after, and the browser proof stayed at 26.
Known property, recorded rather than fixed: `createGoogleGateways` imports the deterministic adapter statically, so that module is present in the production bundle even though `resolveTestMode` makes it unreachable there. It is server-only code and never reaches a browser. Making the import dynamic would turn the factory asynchronous and ripple through every Route Handler, which the authority does not require — the plan asks only that production factories consult the guard first.
...
Task 1 foundation completed in `f05f381` (`chore: scaffold Dockerized Next.js app`); Docker lint/typecheck, Vitest 1/1, production build, and readiness proof passed. Task 2 attendance domain completed in `f4c1f47` with Fix Round 1 configuration-driven status mapping and validation corrections; focused 19/19, full 20/20, Docker lint/typecheck, and diff checks passed. Task 3 adds encrypted Auth.js JWT Google OAuth sessions, identity-preserving refresh-token rotation, server-only token decoding, proxy protection, and English sign-in/sign-out controls; it was committed as `125a0ef` with focused 10/10 and full 30/30 Docker Vitest, Docker lint/typecheck, and Docker production build passing. Security Fix Round 1 rejects refresh-error browser sessions at the proxy, derives Auth.js secure-cookie selection from validated configured/request URLs, and avoids Auth.js evaluation for public pages while protected future UI paths inherit the route-group layout; focused 21/21 and full 41/41 Docker Vitest, Docker lint/typecheck, and Docker production build passed. Security Fix Round 2 removes Auth.js's beta.32 handler wrapper from protected requests: direct Auth.js request handling now enforces redirects for missing/refresh-error sessions while valid unexpired HTTPS sessions continue; focused 23/23 and full 46/46 Docker Vitest, Docker lint/typecheck, and Docker production build passed. Live OAuth remains unattempted pending external credentials and approval. Task 4 adds the typed Drive, Sheets, and Picker boundaries behind gateway interfaces that keep `googleapis` out of every consuming module; it was committed as `255db27` with focused Docker Vitest 27/27. Task 5 Step 1 adds the pure `__APP_CONFIG` version-1 parser and lossless serializer, committed as `659301f` with focused 32/32. Task 7 Steps 1-2 add the shared reference-workbook column contract, the in-memory ExcelJS fixture builders, and bounded XLSX recognition, committed as `07b4cb3` with focused 18/18 and with `jszip@3.10.1` promoted to a declared pinned dev dependency. A combined `npm run verify` passed lint, typecheck, Vitest 123/123, and the production build. Task 5 Steps 2-3 add the config repository and the per-request access policy, committed as `740c5c3` with focused 69/69. Task 6 Steps 1-2 add the deterministic monthly template builder and the create input schema, committed as `00bfe7f` with focused 34/34. The browser-only dashboard folder preference from Task 8 Step 2 was committed as `042264f` with focused 16/16. A combined `npm run verify` passed lint, typecheck, Vitest 210/210 across 20 files, and the production build. Task 6 is complete: the retryable create service and its authenticated route were committed as `528cbf9` with 12/12 service and 7/7 route proof. Task 8 Steps 1-3 add folder-scoped discovery, the dashboard API, and the role-aware dashboard UI, committed as `3e6cdca` with 21/21 discovery proof. `ebe8fa3` fixes a repository-wide test-harness gap that had left Testing Library without cleanup. A combined `npm run verify` exited 0 with Vitest 280/280 across 25 files and the production build. Task 7 is complete: the import service and multipart routes were committed as `c58c813` with 32/32 proof. Task 10 is complete: the member service, route, and English member page were committed as `b1aa63f` with 38/38 proof. Task 11 is complete: authorized attendance reads and exact dirty-range saves were committed as `89745e8` with 81/81 proof, including the additive `RAW` write option that stops free-text notes being evaluated as formulas. `7df86a7` corrects the member setup status vocabulary the create service had invented. A combined `npm run verify` exited 0 with Vitest 388/388 across 33 files and the production build. Task 8 is complete: legacy-file setup was committed as `8e76418` with 48/48 proof. Task 12 is complete: the synchronized timeline and work-block editor was committed as `c17d070` with 77/77 component proof. Task 9 is complete: the create and import wizards were committed as `0e304f1` with 61/61 proof. `d995dce` adds the dashboard entry points the wizards were missing. A combined `npm run verify` exited 0 with Vitest 538/538 across 42 files and the production build. Task 13 is complete. `fe01f9f` adds the production test-bypass guard and the deterministic non-production adapter, `fcfe491` the 26 Playwright workflows, `18b3a19` the reference-workbook contract proof, and `a3e5c61` the product documentation and the Google Cloud runbook. `223c567` splits the three modules that had grown past the repository's 800-line ceiling, with the unchanged test counts as proof that no behavior moved. Every one of the 67 plan steps is checked and the full verification matrix passed command by command.

Live Google proof remains **blocked, not failed**: no OAuth client, Workspace test accounts, or organization approval for `drive.metadata.readonly` exist in this environment, so no real Drive, Sheets, or Picker call has ever been made. Every externally observable rule is proven instead against deterministic fakes and, for the workbook contract, against the real supplied file. `docs/runbooks/google-cloud-setup.md` carries the operator-owned prerequisites and the live smoke checklist that must be run once credentials exist; it states plainly that those steps were not performed here. Until that smoke test runs, the application is proven correct against its specification but unproven against Google itself. Open items carried forward: `sheets-gateway` writes use `USER_ENTERED` so the `=F-G-E` contract works, which means Task 11 must keep free-text notes (column I) from being coerced into formulas or dates; `ConfigMember` sheet, protection, and permission identifiers are nullable so partial setup can be recorded, so `access/policy.ts` must map a null mapping to `NeedsSetupError`/`NeedsRepairError`; and `inspectXlsx` deliberately takes no expected month, so the import service must compare each recognized sheet month against the manager-confirmed month itself; `buildEmployeeSheetPlan` emits an `updateSheetProperties` request that shrinks `rowCount` to the month's row count, which is safe on a freshly added tab but destructive if replayed onto a populated one, so the create and import services must separate creating a tab from reconciling an existing tab rather than replaying the whole plan on retry; `initialize` refuses to overwrite an existing `__APP_CONFIG` unless the caller passes `replaceExisting: true`, which is the switch Task 7 must use to replace an untrusted uploaded config; and `authorizeFile` deliberately does not gate on the file-level `setupState`, because spec section 7.3 scopes the employee check to a valid mapping and the policy already rejects a stale mapping per member, so a manager is never locked out of repairing their own file. Keep this section current during execution with verified outcome, observed commands, limitations, and recovery state. Move the file to `docs/plans/completed/` only after the completion standard in `docs/WORKFLOW.md` is satisfied.
